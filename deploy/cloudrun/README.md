...
The current status/timeout update is revision `fly-training-r57ef0c0c4717`, image `sha256:e7eee3aca42d61b943d930f6c825d23531868001097b1f0383e675102b492e8d`, serving 100% of traffic. Waiting for an assigned trial now displays **Waiting for work**, and a network timeout reports a connection error instead of silently leaving an inactive loop labeled Running. Only `training/client.js` and `training/view.js` changed in the public bundle; the run, physics, lease policy and model/config identity remain unchanged. All 52 focused client tests, the browser control fixture and 15 transport checks passed, with 13 accepted results and the generation-one checkpoint preserved. The [incident record](../../reports/flyheaven-waiting-fix-20260914/diagnosis.json) documents an idle Safari session blocked by the last reserved job in a generation. It automatically reclaimed that job about two seconds after expiry and advanced trial 9 without a restart. That existing session was left running; already-open pages receive the new client on their next reload.
