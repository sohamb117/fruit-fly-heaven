...
class LearningConfig:
    ...
    def physical_horizons(
        ...
        *,
        ...
        """Resolve a declared physical-time protocol, independent of substrate size."""
        values = (
            control_dt,
            discount_seconds,
            advantage_seconds,
        ...
        if any(not math.isfinite(x) or x <= 0 for x in values):
            raise ValueError("Time horizons must be positive and finite")
        if not math.isfinite(burn_in_seconds) or burn_in_seconds < 0:
            raise ValueError("Burn-in time must be finite and nonnegative")
        if advantage_seconds > discount_seconds or sequence_seconds > rollout_seconds:
            raise ValueError("Advantage/sequence horizons exceed discount/rollout horizons")
        result = replace(
            self,
            gamma=math.exp(-control_dt / discount_seconds),
            gae_lambda=math.exp(-control_dt * (1 / advantage_seconds - 1 / discount_seconds)),
            sequence_length=max(1, math.ceil(sequence_seconds / control_dt - 1e-10)),
            burn_in=max(0, math.ceil(burn_in_seconds / control_dt - 1e-10)),
            rollout_steps=max(1, math.ceil(rollout_seconds / control_dt - 1e-10)),
        )
        result.validate()
        return result
