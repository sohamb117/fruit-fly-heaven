...
def compile_plan(config_path, output):
    ...
    def controller_for(task, label, variant, plasticity, seed):
        ...
        base = ControllerConfig(
            ...
            seed,
            normalize_observations=cfg.get("normalize_observations", False),
            normalize_actions=cfg.get("normalize_actions", False),
        )
        ...
        if variant in ("real", "degree_rewired"):
            ...
            result = ControllerConfig(
                adapter,
                SubstrateConfig(kind="adapter_only", seed=seed, dynamics=dyn),
                seed,
                normalize_observations=cfg.get("normalize_observations", False),
                normalize_actions=cfg.get("normalize_actions", False),
            )
    ...
    def add(experiment, task, label, variant, plasticity, seed, regime):
        ...
        if task == "linear_memory":
            ...
            run = RunSpec(
                ...
                controller=controller,
                training=learning.physical_horizons(
                    tasks[task]["dt"], **cfg["ppo_time_horizons"][task]
                )
                if task in cfg.get("ppo_time_horizons", {})
                else learning,
                train_seed=seed,
                device=cfg["device"],
                threads=cfg["threads"],
