...
class AdapterConfig:
    ...
    port_routing: str = "positive"

    def validate(self):
        if self.family not in ADAPTER_FAMILIES:
            raise ValueError("Unknown stateless adapter family")
        if self.port_routing not in ("positive", "balanced_disjoint"):
            raise ValueError("Unknown structural port routing")
        if self.port_routing == "balanced_disjoint" and self.support < 2:
            raise ValueError("Balanced disjoint ports require support >=2")
        if self.family == "anatomical" and self.port_routing != "positive":
            raise ValueError("Anatomical mappings do not use structural port routing")
...
class ControllerConfig:
    ...
    def validate(self):
        ...
        if type(self.normalize_actions) is not bool:
            raise ValueError("normalize_actions must be boolean")
