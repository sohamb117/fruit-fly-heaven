...
class Controller(nn.Module):
    def __init__(self, obs_dim, action_dim, config: ControllerConfig, graph=None):
        ...
        if config.normalize_observations:
            # Frozen training-data statistics travel with every policy checkpoint.
            # They add no policy state and no trainable parameters.
            self.register_buffer("observation_mean", torch.zeros(obs_dim))
            self.register_buffer("observation_scale", torch.ones(obs_dim))
        if config.normalize_actions:
            self.register_buffer("action_center", torch.zeros(action_dim))
    ...
    def normalize_observation(self, observation):
        if not self.config.normalize_observations:
            return observation
        return ((observation - self.observation_mean) / self.observation_scale).clamp(-10, 10)
    ...
    def forward(
        ...
        *,
        ...
        z = self.encoder(self.normalize_observation(observation))
        ...
        if self.config.normalize_actions:
            logits = self.action_center + self.action_scale * logits
