...
for(const precision of ['float64','float32'])test(`Worker ${precision} loads a graph, steps 100 brains and transfers a matrix`,async()=>{
  ...
  try{
    ...
    assert.equal((await call('init',{precision})).precision,precision);
    await assert.rejects(call('init',{precision}),/already initialized/);
    ...
    assert.equal(matrix.precision,precision);
