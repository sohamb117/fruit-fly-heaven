...
**Current combined pair is archival only: do not apply its patch or launch it.** The frozen candidate failed at 262 ms with excessive rotation, zero confirmed takeoffs and zero qualified flight. Its 28 native wing muscle states/forces and motor inputs remained equal to baseline. See `reports/flight-affine-live-pair/status.json` and its pinned result. The successful restrained trim and pure preparation checks do not override this failed release gate. Future component diagnostics require separately declared profiles.

Prepared pair now exists at `reports/flight-affine-live-pair/`, using the accepted `captured-neutral-mouth` calibration and profile commitment `6302ba08230362ad0c2d8c2dc8b17b089c311c3db84e072bed82b530db71c39d`. Both bundles pass the real dev server's `load_bundle` validator without starting a server. The 74 archived source/evidence files were independently rehashed; `verification.json` records checks. No source patch or evaluation has run. The earlier source-only archive is `reports/flight-affine-live-source-archive/`.
