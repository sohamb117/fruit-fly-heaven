...
**The parent selected initial amplitude0.80 and frequency multiplier1 from the declared mechanical reference.** Coordinate2 is zero; the other25 initial coordinates match the migration artifact. The0.81 diagnostic is kept separate and does not initialize training. The source contract is now `reviewed-ready`. No finalizer, server or contribution has run; wait for the active diagnostic to finish and the reviewed combined patch to be installed. Backend hashes and the config/model fingerprint remain unresolved until that installed-source verification.
...
Use global sigma 0.10 and search scales `[0.10,0.25,0.05,1,…,1]` for amplitude, deployment, frequency and 24 steering gains. With initial amplitude 0.80, a perturbation is `0.8*exp(0.01*epsilon)` before bounds:
...
These are marginal Gaussian intervals, not hard bounds or joint 27-dimensional coverage. Parameter limits still clip candidates; several steering gains start at their lower bound. Three-sigma amplitude is 0.77636–0.82436. The exact calculations are in `search-ranges.json`.
...
The following commands are prepared for later execution, after the parent finishes its active native diagnostic and installs the reviewed source. This report has not executed them:
