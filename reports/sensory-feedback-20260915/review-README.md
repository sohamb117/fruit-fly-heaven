...
The page exposes only `off-v1`, `vision-airflow-v1`, `vision-airflow-v2` and `structural-v2` saved frame files. A fresh page selects the latest available recording; Refresh retains the current selection. The structural recording has 140 poses spanning the complete 0–5.5 s native timeline, including the five-second scored endpoint. The three older recordings save only their first 200 poses and omit their final intervals. A coverage notice gives the native and scored timestamp ranges, full evaluated duration, and any missing final interval. Trial score/result refer to the whole evaluation, not the selected pose. Large gaps between saved poses are also shown explicitly.
