...
let renderer, scene, camera, flyMeshes=[], targets=[], selectedRing;
...
function lineBetween(parent,a,b,mat,radius=.08){const vector=b.clone().sub(a);const mesh=new THREE.Mesh(new THREE.CylinderGeometry(radius,radius*.85,vector.length(),5),mat);mesh.position.copy(a).add(b).multiplyScalar(.5);mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),vector.normalize());parent.add(mesh);return mesh;}
...
function initScene(){
  ...
  renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.setClearColor('#e8e7dd');
...
function selectFly(id){selected=id;$('fly-id').value=String(id);if(snapshot)updatePanel();}
function updatePanel(){
  ...
  $('connection').textContent=snapshot.paused?'Brains paused':'100 brains computing';$('live-dot').classList.add('online');
...
async function control(data){
  const response=await fetch('/api/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
  if(!response.ok)throw new Error('Control request failed');
  await poll();
}
async function poll(){
  const forFly=selected;
  try{
    const response=await fetch('/api/state?fly='+forFly);if(!response.ok)throw new Error('Brain server unavailable');
    const next=await response.json();if(forFly!==selected)return;
    snapshot=next;if(snapshot.error)throw new Error(snapshot.error);
    $('error').hidden=true;updatePanel();
  }catch(error){$('connection').textContent='Connection interrupted';$('live-dot').classList.remove('online');$('error').textContent=String(error.message);$('error').hidden=false;}
}

try{
  const response=await fetch('/api/meta');if(!response.ok)throw new Error('Cannot load connectome metadata');meta=await response.json();
  ...
  initScene();await poll();setInterval(()=>{if(!document.hidden)poll();},600);
  $('fly-id').addEventListener('change',()=>{selectFly(Number($('fly-id').value));poll();});
  ...
  $('recenter').addEventListener('click',()=>{following=false;distance=178;elevation=.79;goalLook.set(0,8,0);});
