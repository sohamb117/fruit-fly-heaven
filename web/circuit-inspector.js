...
export function createCircuitInspector(host){
  ...
  function render(){
    ...
    $('visual-status').textContent=current.dataset==='BANC v888'?`Luminance and color input neurons use graded release; zero spikes do not imply inactivity. T4/T5 mean ${number(motion.meanHz,2)} Hz. The graded visual front end remains visible above.`:retina.spikes>0&&motion.spikes===0?'Photoreceptors fired, but T4/T5 produced no spikes in this window. Check the graded visual activity and Vision switch above.':motion.spikes>0?'T4/T5 are firing in the FlyWire graph. They receive inputs from the separate graded visual network; its live activity is shown above.':'No photoreceptor spikes in this window. Check Vision and the eye input rates.';
    ...
    if(current.dataset==='BANC v888')$('motor-status').textContent=channel.spikes>0?`${channel.active} / ${channel.n} motor cells fired · mean ${number(channel.meanHz,2)} Hz. Individual motor-neuron release feeds muscle activation and force; channel means summarize the population.`:`No motor spikes in this window. ${channel.cells.every(c=>c.voltageMv<c.thresholdMv)?'Sampled voltages remain below each cell’s firing threshold.':'A sampled cell has reached its configured threshold.'} Muscle activation can decay after earlier spikes.`;
    $('threshold').textContent=current.dataset==='BANC v888'?`Cell-specific rest and threshold · mean external, tonic and adaptation current ${number(channel.meanDriveMv)} pA · input ranking in ${current.incomingUnit}`:`Rest ${current.restMv} mV · firing threshold ${current.thresholdMv} mV · mean net synaptic drive ${number(channel.meanDriveMv)} mV`;
    $('cells').replaceChildren(...channel.cells.map(c=>row(`${c.type} · ${c.side}`,c.graded?'graded':number(c.rateHz,2)+' Hz',number(c.voltageMv)+' mV',c.graded?'—':number((c.thresholdMv??current.thresholdMv)-c.voltageMv)+' mV',c.root_id)));
