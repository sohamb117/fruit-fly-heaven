...
// camera, overlays and trails are excluded. RGB feeds spectral reconstruction;
// the original luminance plane is preserved for the trained motion model.
export function createRetinalCamera(renderer,scene,{width=32,height=16,eye_yaw_degrees=60,vertical_fov_degrees=120}={}){
  ...
  return {
    capture(f,mesh,exclude=[]){
      const pixels=new Uint8Array(width*height*2),rgb=new Uint8Array(width*height*6),before=renderer.getRenderTarget(),shadow=renderer.shadowMap.autoUpdate;
      ...
      try{
        ...
        for(const [i,side]of [-1,1].entries()){
          ...
          for(let y=0;y<height;y++)for(let x=0;x<width;x++){
            ...
            const at=i*width*height+y*width+x;
            rgb.set(rgba.subarray(p,p+3),at*3);
            pixels[at]=Math.round(.2126*rgba[p]+.7152*rgba[p+1]+.0722*rgba[p+2]);
          }
        }
      }finally{renderer.setRenderTarget(before);renderer.shadowMap.autoUpdate=shadow;exclude.forEach((o,i)=>o.visible=visibility[i]);}
      return {id:f.id,sequence:++sequence,bodyTime:f.bodyTime,pixels,rgb};
    },
