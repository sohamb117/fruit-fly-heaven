...
async function setup(){
 const config=structuredClone(original);config.stage='takeoff';config.durationSeconds=config.stages.find(s=>s.id===config.stage).durationSeconds;config.optimizer.populationPairs=1;
 ...
 const shared=checkpoint(config,configHash,config.parameters.map(p=>Math.max(p.min,Math.min(p.max,.1))),9,'flight',{status:'unverified'});
 const status={modelFingerprint:config.modelFingerprint,configHash,generation:9,checkpoint:shared};
 const job={jobId:'shared-fixture',leaseToken:'fixture-token',modelFingerprint:config.modelFingerprint,configHash,parameters:[...shared.parameters],stage:'flight',seed:1,durationSeconds:config.stages.find(s=>s.id==='flight').durationSeconds,generation:9,pairId:'shared-pair',sign:1};
...
test('shared work preserves the exact local checkpoint and pending optimizer round on return',async()=>{
 ...
 c.state.generation=2;c.round=makeGeneration(c.parameters,2,config,'takeoff');const round=c.round;
 c.savedValidation={stage:'takeoff',meanReturn:-1,passed:false};const validation=c.savedValidation;
 ...
 assert.equal(c.state.stage,'flight');assert.equal(c.state.generation,9);assert.equal(c.state.parameters[0],.1);
 const saved=JSON.parse(data.get(c.key)).checkpoint;assert.equal(saved.stage,'takeoff');assert.equal(saved.generation,2);assert.deepEqual(saved.parameters,config.parameters.map(p=>p.initial));
 ...
 await c.start({mode:'local'});await c.loop;assert.deepEqual(restored,{stage:'takeoff',generation:2,parameters:config.parameters.map(p=>p.initial)});assert.equal(c.round,round);assert.equal(c.savedValidation,validation);await c.dispose();
});
...
test('import and stage changes clear validation evidence for the previous vector or task',async()=>{
 ...
 c.savedValidation={stage:'flight',passed:true};c.testValidation={stage:'flight',passed:true};c.state.checkpointStatus='locally-tested';c.runLocal=async()=>{};await c.start({mode:'local',stage:'takeoff'});await c.loop;assert.equal(c.savedValidation,null);assert.equal(c.testValidation,null);assert.equal(c.state.checkpointStatus,'unverified');await c.dispose();
});
test('a newly accepted vector invalidates earlier-stage badges, and final stage alone is not called all stages',async()=>{
 const {c}=await setup();c.state.stage='landing';c.completedStages.add('takeoff');c.state.curriculum=c.config.stages.map(s=>({...s,status:s.id==='takeoff'?'validated':'available'}));c.savedValidation={stage:'landing',meanReturn:-10,passed:false};
 await c.start({mode:'local',stage:'landing'});await c.loop;assert(c.parameters[0]>0);assert.equal(c.completedStages.has('takeoff'),false);assert.equal(c.completedStages.has('landing'),true);assert.equal(c.state.curriculum.find(s=>s.id==='takeoff').status,'available');assert.match(c.state.message,/passed local validation/);assert.doesNotMatch(c.state.message,/All curriculum stages/);await c.dispose();
});
